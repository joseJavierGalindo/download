...
def processUploadFiles(filename,filesize,files,update,bot,message,thread=None):
    try:
        bot.editMessageText(message,'⏏️Preparando Para Subir⏏️...')
        evidence = None
        fileid = None
        ...
        if loged:
            ...
            bot.editMessageText(message,'🚫PROBLEMAS EN LA PÁGINA 🚫')
        return None
    except:
        bot.editMessageText(message,'🚫PROBLEMAS EN LA PÁGINA🚫')
...
def processFile(update,bot,message,file,thread=None):
    ...
    if file_size > max_file_size:
        ...
        file_upload_count = 1
    bot.editMessageText(message,'📄ENVIANDOTE LINKS Y TXT 📎...')
    evidname = ''
    files = []
    if client:
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username not in config.PV_USERS:
            bot.sendMessage(update.message.chat.id,'🚫 POR FAVOR PARA HACERME FUNCIONAR ESCRIBALE A MIS DUEÑOS @troppsss y @Tito_01Alex😁')
            return
        ...
        if config.getUser(username) is None:
            ...
            reply_msg+= 'Nesesita configurar su cuenta\n'
            ...
            if '/myuser' != msgText and '/tutorial' not in msgText and '/cancel' not in msgText and '/account' not in msgText and '/host' not in msgText and '/repo' not in msgText:
                configmsg = '😬Primero configure su cuenta por favor 😬\n⚙️/myuser Para Configurar⚙️'
        ...
        if '/adduser' in msgText:
            ...
            if isadmin:
                ...
                bot.sendMessage(update.message.chat.id,'🚫USTED NO TIENE PERMISO ESCRIBA A @Tito_01Alex🚫')
            return
        ...
        if '/start' in msgText:
            start_msg = 'Bot     : @xfreedownload_bot\n'
            start_msg+= 'DUEÑO🔐  : @Tito_01Alex\n'
            ...
            if loged:
                ...
                client.logout()
                bot.editMessageText(message,'🗑️ARCHIVO ELIMINADO🗑️')
            else:
                bot.editMessageText(message,'🚫Error y Causas🤒🥸\n1-Revise su Cuenta\n2-Servidor Desabilitado: '+client.path)
            ...
            if 'youtube' in url or 'youtu.be' in url:
                try:
                    ...
                    if data:
                        ...
                        bot.editMessageText(message,'🚫LO SIENTO ESTE LINK NO ME FUNCIONA🚫')
                        return
                except:
                    bot.editMessageText(message,'🚫LO SIENTO NO SOPORTO CARPETAS DE MEDIAFIRE🚫')
                    return
            if 'mediafire' in url:
                try:
